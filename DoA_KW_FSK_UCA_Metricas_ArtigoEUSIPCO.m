...
TotalSim = nSNR*nISR*nRadius*nPhi*nPhi;
iTotal = 0;
% Armazenamento
        ...
        for iRadius = 1:nRadius
            ...
            for iPhi = 1:nPhi
                ...
                for iiPhi = 1:nPhi
                    ...
                    [X, q, r_int, bits, pam_rrc_tx, pam_rect, sym_tx, qn, taus_sig, taus_int] = ...
                        utils.simulate_fsk_data_uca(M, r, lambda, phi_sig_deg, phi_int_deg, ...
                        theta_sig_deg, theta_int_deg, SNR_dB, ISR_dB, N, fs, Rs, sps, alpha, span, fd);
                    ...
                    a_sig = utils.steering_vec_uca(M, r, lambda, theta_sig_deg, phi_sig_deg);  % vetor de direção Mx1
                    ...
                    for k = 1:Nnull
                        a_null = utils.steering_vec_uca(M, r, lambda, theta_sig_deg, phi_nulls(k));  % vetor de direção Mx1
...
BER_mean = mean(BER, 5);   % média ao longo da 5ª dimensão (trials)
EVM_mean = mean(EVM, 5);
...
for iISR = 1:nISR
    fig = figure('Name', "BER vs SNR | ISR=" + string(range_ISR_dB(iISR)), ...
        'NumberTitle', 'off');
    hold on; grid on;
    ...
    for iRadius = 1:nRadius
        ...
        for m = 2:nMethods
            ...
            plot(range_SNR_dB, squeeze(BER_mean(m,:,iISR,iRadius))*100, markers(m), 'LineWidth', 4, 'LineStyle', line_style(iRadius));
