...
% medindo BER/EVM no payload, em DOIS sweeps independentes (mesmos graficos):
%
%   SWEEP 1: varre SNR  (ISR fixo)   -> BER/EVM vs SNR
%   SWEEP 2: varre ISR  (SNR fixo)   -> BER/EVM vs ISR
%
...
%   DoA p/ apontar: OS 4 METODOS (KW/DAS/CAPON/MUSIC). MUSIC com 1 FONTE.
%   Beamformers: DAS e Capon (MPDR).
%
%   Curvas por painel (2x2: linhas=beamformer, colunas=estrategia que usa DoA):
%     4 metodos (sem-comp / inversao) + referencias No-MC, Ideal, No-Comp + BF-KW
%   Figuras por sweep: BER (fracao log), BER (% linear), EVM.
%   Figura unica: beampatterns (cenario representativo).
% =========================================================================
...
% --- valores FIXOS e RANGES de cada sweep ---
ISR_fix      = -2;                          % ISR fixo durante a varredura de SNR
SNR_fix      =  6;                          % SNR fixo durante a varredura de ISR
range_SNR_dB = [-12 -9 -6 -3 0 3 6 9 12];   % usado no sweep de SNR
range_ISR_dB = [-15 -12 -9 -6 -3 0 3 6 9 12 15];  % usado no sweep de ISR
...
%% ---- C_true (normalizado p/ preservar energia: rede de acoplamento passiva) ----
...
fprintf('C_true (raio=%.2f lambda, cond=%.1f, norm s=%.3f, <||Ca||^2>=%.2f->%d), sep=%d deg, K_kw=%d, payload=%d\n', ...
    r/lambda, cond(C_true), s_norm, gbar, M, sep_deg, K_kw, payload_len);
...
% --- definicao dos dois sweeps ---
sweeps = struct( ...
    'name',   {'SNR',          'ISR'}, ...
    'vals',   {range_SNR_dB,   range_ISR_dB}, ...
    'xlab',   {'SNR (dB)',     'ISR (dB)'}, ...
    'fixname',{'ISR',          'SNR'}, ...
    'fixval', {ISR_fix,        SNR_fix}, ...
    'suf',    {'snr',          'isr'});

%% =======================================================================
%  LACO PRINCIPAL: um sweep por vez
%% =======================================================================
for isw = 1:numel(sweeps)
    sw = sweeps(isw);  xvals = sw.vals;  nX = numel(xvals);
    fixstr = sprintf('%s=%+d dB', sw.fixname, sw.fixval);
    fprintf('\n######## SWEEP %s  (%s fixo) ########\n', sw.name, fixstr);

    % ---- Acumuladores ----
    sBER_id  = zeros(nBF,nX);    sEVM_id  = zeros(nBF,nX);    % No-MC
    sBER_idl = zeros(nBF,nX);    sEVM_idl = zeros(nBF,nX);    % Ideal
    sBER_kw  = zeros(nBF,nX);    sEVM_kw  = zeros(nBF,nX);    % BF-KW
    sBER_sc  = zeros(nBF,nM,nX); sEVM_sc  = zeros(nBF,nM,nX); % sem comp por metodo
    sBER_iv  = zeros(nBF,nM,nX); sEVM_iv  = zeros(nBF,nM,nX); % inversao por metodo
    cnt = zeros(1,nX);

    rng(777,'twister');             % mesma sequencia de ruido nos dois sweeps
    t0 = tic;
    for ix = 1:nX
        if strcmp(sw.name,'SNR'), SNR_dB = xvals(ix);  ISR_run = sw.fixval;
        else,                      SNR_dB = sw.fixval;  ISR_run = xvals(ix);  end
        fprintf('\n== %s=%+d dB (%s) ==\n', sw.name, xvals(ix), fixstr);
        for ia = 1:n_angles
            phi_sig = phi_set(ia);
            ...
            for itr = 1:n_trials
                [Xtot, y_ref, k0t, Xsig, Xint, Xn, bits_full, sym_full, bits_pre, ~] = ...
                    ...
                    SNR_dB, ISR_run, K, K_kw, fs, Rs, sps, alpha, span, fd, interf_t, k0);
                q = y_ref(:);  bits_full = bits_full(:);  sym_full = sym_full(:);
                nsym_pre = numel(bits_pre);
                win = k0t : k0t + K_kw - 1;
                act = k0t : K;

                X_ideal_full   = Xtot;                       % sem acoplamento
                X_coupled_full = C_true*(Xsig + Xint) + Xn;  % com acoplamento
                Xk_i = X_ideal_full(:, win);
                Xk_c = X_coupled_full(:, win);
                b_hat = Xk_c*conj(q)/(q'*q);                 % manifold do SOI
                a_true = utils.steering_vec_uca(M, r, lambda, theta_sig_deg, phi_sig);

                % self-cal (KW) -> Chat -> desacopla sinal todo
                c_fin = selfcal_kw(Xk_c, b_hat, q, M, r, lambda, theta_sig_deg, beta_uca, phi_grid_deg, u, maxIter);
                D = safe_inv(reconstruct_C(c_fin, M));
                Yd_full = D*X_coupled_full;
                Yd_pre  = Yd_full(:, win);

                % DoA p/ apontar (por metodo)
                ...
                for ib = 1:nBF
                    ...
                    sBER_id(ib,ix)=sBER_id(ib,ix)+b;    sEVM_id(ib,ix)=sEVM_id(ib,ix)+e;
                    [b,e] = bf_demod(X_coupled_full, Xk_c,  C_true*a_true, bf, act, nsym_pre, bits_full, sym_full, Rs, sps, alpha, span, fd);
                    sBER_idl(ib,ix)=sBER_idl(ib,ix)+b;  sEVM_idl(ib,ix)=sEVM_idl(ib,ix)+e;
                    [b,e] = bf_demod(X_coupled_full, Xk_c,  b_hat,         bf, act, nsym_pre, bits_full, sym_full, Rs, sps, alpha, span, fd);
                    sBER_kw(ib,ix)=sBER_kw(ib,ix)+b;    sEVM_kw(ib,ix)=sEVM_kw(ib,ix)+e;
                    for m = 1:nM
                        [b,e] = bf_demod(X_coupled_full, Xk_c,  a_cm{m}, bf, act, nsym_pre, bits_full, sym_full, Rs, sps, alpha, span, fd);
                        sBER_sc(ib,m,ix)=sBER_sc(ib,m,ix)+b;  sEVM_sc(ib,m,ix)=sEVM_sc(ib,m,ix)+e;
                        [b,e] = bf_demod(Yd_full,        Yd_pre, a_dm{m}, bf, act, nsym_pre, bits_full, sym_full, Rs, sps, alpha, span, fd);
                        sBER_iv(ib,m,ix)=sBER_iv(ib,m,ix)+b;  sEVM_iv(ib,m,ix)=sEVM_iv(ib,m,ix)+e;
                    end
                end
                cnt(ix) = cnt(ix) + 1;
            end
        end
        fprintf('  %s=%+d feito | %.0fs\n', sw.name, xvals(ix), toc(t0));
    end

    % ---- Consolidacao ----
    cS = reshape(cnt,1,nX);  cS3 = reshape(cnt,1,1,nX);
    ...
    BER_nc = reshape(BER_sc(:,iCap,:), nBF, nX);  EVM_nc = reshape(EVM_sc(:,iCap,:), nBF, nX);
    flrB = @(x) max(x, 0.5/(max(cnt)*nsym_pay));     % piso ~ 1 erro no payload

    %% ============ FIG 1: BER (fracao log + porcentagem linear) ============
    allB = flrB([BER_sc(:); BER_iv(:); BER_id(:); BER_idl(:); BER_kw(:)]);
    ber_modes = { 1,   'BER (payload)',    sprintf('pipeline_BER_vs_%s.png',sw.suf),     'log',    'fracao'; ...
                  100, 'BER (%, payload)', sprintf('pipeline_BER_pct_vs_%s.png',sw.suf), 'linear', '%' };
    for imode = 1:size(ber_modes,1)
        sc    = ber_modes{imode,1};
        ylab  = ber_modes{imode,2};
        fout  = ber_modes{imode,3};
        yscl  = ber_modes{imode,4};
        unit  = ber_modes{imode,5};
        ...
        for ib = 1:nBF
            for js = 1:2
                ...
                for m = 1:nM
                    plot(xvals, sc*flrB(squeeze(Bm(ib,m,:))), mkM{m}, 'Color',colsM(m,:), ...
                        'LineWidth',1.7,'MarkerFaceColor',colsM(m,:),'MarkerSize',6,'DisplayName',methods{m});
                end
                plot(xvals, sc*flrB(BER_id(ib,:)),  'k--','LineWidth',1.6,'DisplayName','No-MC');
                plot(xvals, sc*flrB(BER_idl(ib,:)), 'k-', 'LineWidth',2.4,'DisplayName','Ideal');
                plot(xvals, sc*flrB(BER_nc(ib,:)),  ':', 'Color',[0.55 0.55 0.55],'LineWidth',2.2,'DisplayName','No-Comp (Capon)');
                plot(xvals, sc*flrB(BER_kw(ib,:)),  '-.','Color',[0.60 0 0.70],'LineWidth',2.2,'DisplayName','BF-KW');
                set(gca,'YScale',yscl); xlabel(sw.xlab); ylabel(ylab); xticks(xvals);
                if strcmp(yscl,'linear')
                    ytop = min(100, max(20, ceil(max(100*allB)/10)*10));
                    ylim([0 ytop]); yticks(0:10:ytop);
                else
                    ylim([min(allB)*0.7, 1]);
                end
        ...
        sgtitle(sprintf('BER (payload, %s) vs %s  (raio=%.2f\\lambda, %s, sep=%d^o)', ...
            unit, sw.name, r/lambda, fixstr, sep_deg),'FontWeight','bold');
    ...
    for ib = 1:nBF
        for js = 1:2
            ...
            for m = 1:nM
                plot(xvals, squeeze(Em(ib,m,:)), mkM{m}, 'Color',colsM(m,:), ...
                    'LineWidth',1.7,'MarkerFaceColor',colsM(m,:),'MarkerSize',6,'DisplayName',methods{m});
            end
            plot(xvals, EVM_id(ib,:),  'k--','LineWidth',1.6,'DisplayName','No-MC');
            plot(xvals, EVM_idl(ib,:), 'k-', 'LineWidth',2.4,'DisplayName','Ideal');
            plot(xvals, EVM_nc(ib,:),  ':', 'Color',[0.55 0.55 0.55],'LineWidth',2.2,'DisplayName','No-Comp (Capon)');
            plot(xvals, EVM_kw(ib,:),  '-.','Color',[0.60 0 0.70],'LineWidth',2.2,'DisplayName','BF-KW');
            xlabel(sw.xlab); ylabel('EVM (dB, payload)'); xticks(xvals); ylim(ylE);
            title(sprintf('%s  |  %s', bf_list{ib}, strat_name{js}));
            if ib==1 && js==1, legend('Location','northeast'); end
        end
    end
    sgtitle(sprintf('EVM (payload) vs %s  (raio=%.2f\\lambda, %s, sep=%d^o)', ...
        sw.name, r/lambda, fixstr, sep_deg),'FontWeight','bold');
    exportgraphics(fig, fullfile(outDir,sprintf('pipeline_EVM_vs_%s.png',sw.suf)),'Resolution',170);
    matlab2tikz(fullfile(outDir,sprintf('pipeline_EVM_vs_%s.tex',sw.suf)), 'width','\figurewidth','height','\figureheight');

    %% ---- Resumo numerico (Capon) ----
    ibC = find(strcmp(bf_list,'CAPON'));
    fprintf('\n===== BER (%%) payload, Capon, INVERSAO por metodo, vs %s (%s) =====\n', sw.name, fixstr);
    fprintf('%-10s', sw.name); fprintf('%9d', xvals); fprintf('\n');
...
% UM unico pacote. Padrao espacial efetivo no canal real h(phi)=C_true*a(phi),
% normalizado ao maximo. Usa o ISR fixo do sweep de SNR.
rng(7,'twister');
phi_bp_sig = 40;  SNR_bp = 6;  ISR_bp = -6; phi_bp_int = 90;
[Xtot_b, yb, k0b, Xs_b, Xi_b, Xn_b] = utils.simulate_data_uca_v3(M, r, lambda, ...
    phi_bp_sig, phi_bp_int, theta_sig_deg, theta_sig_deg, SNR_bp, ISR_bp, K, K_kw, ...
    fs, Rs, sps, alpha, span, fd, interf_t, k0);
qb = yb(:);  winb = k0b:k0b+K_kw-1;
Xc_full_b = C_true*(Xs_b + Xi_b) + Xn_b;
...
sgtitle(sprintf('Beampatterns efetivos no canal real  (SOI=%d^o, interf=%d^o, SNR=%+d dB, ISR=%+d dB, raio=%.2f\\lambda)', ...
    phi_bp_sig, phi_bp_int, SNR_bp, ISR_bp, r/lambda),'FontWeight','bold');
...
%% =================== FIG 3b: ESPECTROS DE DoA (mesmo cenario) ===================
% Espectro espacial dos metodos que VARREM o angulo (DAS, Capon, MUSIC),
% calculado no MESMO preambulo acoplado Xk_cb usado nos beampatterns. Mostra
% onde cada metodo poe o pico -> da' p/ ver se acham o SOI ou travam no
% interferente / se o acoplamento desloca o pico. (KW nao entra: nao varre
% angulo, usa a correlacao com a forma de onda conhecida.)
R_bp = (Xk_cb*Xk_cb')/size(Xk_cb,2);
Rdl  = R_bp + 1e-6*trace(R_bp)/M*eye(M);
P_das   = real(sum(conj(A_plot).*(R_bp  *A_plot),1));
P_capon = 1./max(real(sum(conj(A_plot).*(Rdl\A_plot),1)),eps);
Rsym = (R_bp+R_bp')/2; [Vb,Db2] = eig(Rsym); [~,ixb] = sort(real(diag(Db2)),'descend');
Enb = Vb(:,ixb(2:end));  projb = Enb'*A_plot;        % MUSIC com 1 FONTE
P_music = 1./max(real(sum(conj(projb).*projb,1)),eps);
nrm = @(p) 10*log10(p/(max(p)+eps));                 % normaliza ao maximo (0 dB)

fig = figure('Color','w','Position',[60 60 940 520]); hold on; grid on;
plot(phi_grid_deg, nrm(P_das),   '-','Color',[0.00 0.45 0.74],'LineWidth',1.8,'DisplayName','DAS');
plot(phi_grid_deg, nrm(P_capon), '-','Color',[0.49 0.18 0.56],'LineWidth',1.8,'DisplayName','Capon');
plot(phi_grid_deg, nrm(P_music), '-','Color',[0.85 0.33 0.10],'LineWidth',1.8,'DisplayName','MUSIC');
xline(phi_bp_sig,'--','Color',[0 0.6 0],   'LineWidth',1.6,'DisplayName','SOI');
xline(phi_bp_int,'--','Color',[0.85 0 0],  'LineWidth',1.6,'DisplayName','interferente');
xlim([-180 180]); xticks(-180:45:180); ylim([-10 2]);
xlabel('azimute \phi (graus)'); ylabel('espectro normalizado (dB)');
title(sprintf('Espectros de DoA no preambulo acoplado  (SOI=%d^o, interf=%d^o, SNR=%+d dB, ISR=%+d dB, raio=%.2f\\lambda)', ...
    phi_bp_sig, phi_bp_int, SNR_bp, ISR_bp, r/lambda));
legend('Location','southwest');
exportgraphics(fig, fullfile(outDir,'pipeline_doa_spectra.png'),'Resolution',170);
matlab2tikz(fullfile(outDir,'pipeline_doa_spectra.tex'), 'width','\figurewidth','height','\figureheight');
