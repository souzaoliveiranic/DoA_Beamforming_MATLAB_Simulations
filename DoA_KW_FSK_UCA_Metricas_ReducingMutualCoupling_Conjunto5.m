...
% =========================================================================
% Flags de plots opcionais (default false).
%   plot_frobenius        -> plots de RMSE de Frobenius da matriz C
%   plot_accuracy         -> scatter de acuracia agregada (DoA vs Frobenius)
%   plot_selfcal_vs_iter  -> RMSE de DoA vs numero de iteracoes do self-cal
% =========================================================================
plot_frobenius       = false;
plot_accuracy        = false;
plot_selfcal_vs_iter = true;

% =========================================================================
% Modos dedicados de SCATTER (rodam APOS o sweep normal). Cada um varre um
% eixo de "sujeira" e gera uma grade de mini-scatters (linhas = metodos de
% self-cal SC-DAS/CAPON/MUSIC/KW; colunas = niveis crescentes do efeito).
% Eixos de cada celula: erro de theta (X) vs erro de phi (Y), centrado em 0.
%   run_scatter_sync    -> varia o desalinhamento Delta (sincronizacao)
%   run_scatter_noncirc -> varia a nao-circularidade de C
% =========================================================================
run_scatter_sync    = false;
run_scatter_noncirc = false;
n_scatter_real      = 500;   % realizacoes Monte Carlo por celula
% Niveis varridos (colunas da grade):
scatter_sync_syms   = [0, 5, 10, 20, 30, 40];          % Delta em SIMBOLOS (x sps -> amostras)
scatter_noncirc_lvl = [0, 0.02, 0.05, 0.1, 0.2, 0.4];  % C_noncirc_level
...
% Cenarios 11-14 = NC (No Calibration): mesmos metodos de DoA dos self-cal
% (6-9) mas SEM compensar o sinal. Permite comparar self-cal vs sem-cal.
range_coupling = [0, 1, 2, 4, 9, 14];
...
labels_cmp = ["No coupling (ideal)", ...
              ...
              "Coupling, no-cal DAS", ...
              "Coupling, no-cal CAPON", ...
              "Coupling, no-cal MUSIC", ...
              "Coupling, no-cal KW"];
for ic = 1:nCoupling
    for iI = 1:nInits
                ...
                for iPhi = 1:nPhi
                    ...
                    for iiPhi_local = 1:numel(iiPhi_range)
                        ...
                        if(phi_int_deg ~= phi_sig_deg)
                            ...
                            Ksrc = 1;   % ajuste se tiver mais fontes
                            ...
                            if ~use_doa_estimate_in_bf
                                ...
                                switch Coupling
                                    ...
                                    case 11, phi_bf_target = phi_DAS;   % NC-DAS  (sem calibracao)
                                    case 12, phi_bf_target = phi_MVDR;  % NC-CAPON
                                    case 13, phi_bf_target = phi_MUSIC; % NC-MUSIC
                                    case 14, phi_bf_target = phi_hat_deg;% NC-KW
                                    otherwise
                            ...
                            switch Coupling
                                ...
                                case 11, name_string = 'Coupling (No-cal DAS)';
                                case 12, name_string = 'Coupling (No-cal CAPON)';
                                case 13, name_string = 'Coupling (No-cal MUSIC)';
                                case 14, name_string = 'Coupling (No-cal KW)';
                            end
                             % ----- PLOT DOA -----
...
for iInitPlot = 1:nInits
    ...
    if ~isempty(sc_indices_in_results)
        ...
        for ic_s = 1:nCoupling
            ...
            if coup_code < 2 || coup_code >= 10, continue; end   % pula No-MC, No-Comp e cenarios sem ^C (10-14)
        ...
        exportgraphics(fig_scatter, fullfile(outDir, ...
            sprintf('scatter_doa_vs_frobenius_init_%s.png', init_tag)), ...
                       'Resolution', 200);
        end   % --- if plot_frobenius (scatter doa-vs-frobenius) ---
...
cmap_cen = [
    ...
    0.95 0.50 0.10;   % 11 NC-DAS    laranja (= SC-DAS, estilo diferente)
    0.55 0.10 0.65;   % 12 NC-CAPON  roxo    (= SC-CAPON)
    1.00 0.85 0.15;   % 13 NC-MUSIC  amarelo (= SC-MUSIC)
    0.95 0.30 0.65;   % 14 NC-KW     rosa    (= SC-KW)
];
% Self-cal (6-9) solido; NC sem-calibracao (11-14) tracejado, mesma cor.
styles_cen = {':',':','--','--','-.','-.','-','-','-','-','-','--','--','--','--'};
markers_cen = {'none','none','s','d','d','s','o','o','o','o','^','s','s','s','s'};
short_labels_cen = {'No-MC', 'No-Comp', 'Ideal', 'Pert', ...
                    'KW-1D', 'KW-MD', 'SC-DAS', 'SC-CAPON', 'SC-MUSIC', 'SC-KW', ...
                    'BF-Direto', 'NC-DAS', 'NC-CAPON', 'NC-MUSIC', 'NC-KW'};
...
for iInitPlot_agg = 1:nInits
    ...
    for ic_m = 1:nCoupling
        ...
        switch cc
            ...
            case 11, method_idx(ic_m) = 2;  % NC-DAS    usa DAS
            case 12, method_idx(ic_m) = 3;  % NC-CAPON  usa MVDR
            case 13, method_idx(ic_m) = 4;  % NC-MUSIC  usa MUSIC
            case 14, method_idx(ic_m) = 1;  % NC-KW     usa KW
    ...
    for iSNR_fix = 1:nSNR
        ...
        for ic = 1:nCoupling
            coup_code = range_coupling(ic);
            if coup_code == 10, continue; end   % BF-Direto: sem DoA p/ apontar
    ...
    for iISR_fix = 1:nISR
        ...
        for ic = 1:nCoupling
            coup_code = range_coupling(ic);
            if coup_code < 2 || coup_code >= 10, continue; end
    ...
    end   % --- if plot_frobenius (rmse_frobenius vs SNR/ISR) ---
    ...
    exportgraphics(fig_h, fullfile(outDir, ...
        ...
        'Resolution', 200);
    end   % --- if plot_accuracy ---
end


% #########################################################################
% #  EXPERIMENTOS DEDICADOS (rodam APOS o sweep normal)                   #
% #  Cada bloco e' autocontido: tem seu proprio loop Monte Carlo,         #
% #  regenera sinal/matriz e reusa simulate_data_uca_v3 + estimate_C_selfcal.
% #  Ponto operacional fixo: SNR=3 dB, ISR=-3 dB, init self-cal=identity. #
% #########################################################################

% Parametros comuns dos experimentos dedicados
exp_SNR_dB   = 3;
exp_ISR_dB   = -3;
exp_radius_m = range_radius(1) * lambda;
exp_theta    = 90;                     % fontes no plano horizontal
sc_labels    = {'SC-DAS', 'SC-CAPON', 'SC-MUSIC', 'SC-KW'};
sc_meth_list = {'DAS', 'CAPON', 'MUSIC', 'KW'};
beta_uca_exp = 2*pi*(0:M-1)'/M;


% =========================================================================
% ITEM 2: RMSE de DoA vs NUMERO DE ITERACOES do self-cal
% Mostra se algum metodo piora com excesso de iteracoes (over-iteration).
% Para cada metodo, roda n_scatter_real realizacoes, coleta a DoA estimada
% por iteracao (history.phi_per_iter) e calcula o RMSE por iteracao.
% =========================================================================
if plot_selfcal_vs_iter
    fprintf('\n[Experimento] RMSE de DoA vs iteracoes do self-cal (varrendo ISR)...\n');
    n_rep_iter = n_scatter_real;
    max_it     = selfcal_max_iter;
    nMet_it    = numel(sc_meth_list);
    nISR_it    = numel(range_ISR_dB);

    % Acumuladores: soma de erro^2 por (metodo, iteracao, ISR) e contagem
    err2_iter = zeros(nMet_it, max_it, nISR_it);
    cnt_iter  = zeros(nMet_it, max_it, nISR_it);
    % TESTE DE VALIDADE DA METRICA (opcao 2): acumula o residuo observavel
    % por (metodo, iteracao, ISR) para comparar com o erro de DoA verdadeiro.
    resid_iter     = zeros(nMet_it, max_it, nISR_it);
    cnt_resid_iter = zeros(nMet_it, max_it, nISR_it);

    Cmat = Coupling_matrices_real(:,:,1);
    for iISR_it = 1:nISR_it
        isr_now = range_ISR_dB(iISR_it);
        fprintf('  ISR=%+d dB (%d/%d)...\n', isr_now, iISR_it, nISR_it);
        for rep = 1:n_rep_iter
            [phi_sig_d, phi_int_d] = deal_local_angles(min_sep_deg);
            % Sinal (modelo de preambulo) com acoplamento real, SNR fixo, ISR variavel
            [Xc, y_ref_c, k0c] = utils.simulate_data_uca_v3(M, exp_radius_m, lambda, ...
                phi_sig_d, phi_int_d, exp_theta, exp_theta, ...
                exp_SNR_dB, isr_now, K, K_kw, fs, Rs, sps, alpha, span, fd, interf_type, []);
            Xc = Cmat * Xc;                          % aplica acoplamento
            Xk = Xc(:, k0c:k0c+K_kw-1);              % janela do preambulo (Delta=0)
            qv = y_ref_c(:);

            for im = 1:nMet_it
                [~, ~, ~, ~, hist_it] = estimate_C_selfcal(Xk, qv, M, exp_radius_m, lambda, ...
                    sc_meth_list{im}, selfcal_grid_deg, max_it, 0, 0, Cmat, ...
                    selfcal_damping, 'identity', true);   % tol=0 + force_full_iters=true
                ni = hist_it.n_iter;
                for it = 1:ni
                    e = abs(hist_it.phi_per_iter(it) - phi_sig_d);
                    e = min(e, 360 - e);            % erro circular
                    err2_iter(im, it, iISR_it) = err2_iter(im, it, iISR_it) + e^2;
                    cnt_iter(im, it, iISR_it)  = cnt_iter(im, it, iISR_it) + 1;
                end
                if isfield(hist_it,'resid_per_iter')
                    for it = 1:numel(hist_it.resid_per_iter)
                        resid_iter(im, it, iISR_it) = resid_iter(im, it, iISR_it) + ...
                            hist_it.resid_per_iter(it);
                        cnt_resid_iter(im, it, iISR_it) = cnt_resid_iter(im, it, iISR_it) + 1;
                    end
                end
            end
        end
    ...
    rmse_iter = sqrt(err2_iter ./ max(cnt_iter, 1));
    rmse_iter(cnt_iter == 0) = NaN;
    resid_mean = resid_iter ./ max(cnt_resid_iter, 1);
    resid_mean(cnt_resid_iter == 0) = NaN;

    % DIAGNOSTICO: quantas iteracoes o self-cal realmente rodou? Se ~1, o
    % force_full_iters NAO esta' ativo (versao antiga em cache -> rode
    % 'clear functions' no MATLAB) e por isso o residuo so' tem 1 ponto.
    cnt_by_iter = squeeze(sum(sum(cnt_iter, 1), 3));   % total por iteracao
    fprintf('\n  [diag] Realizacoes que atingiram cada iteracao:\n  ');
    for it = 1:max_it
        fprintf('it%d=%d ', it, round(cnt_by_iter(it)));
    end
    fprintf('\n');
    if max_it >= 2 && cnt_by_iter(2) == 0
        warning(['Self-cal rodou so 1 iteracao -> force_full_iters inativo. ' ...
                 'Rode "clear functions" no MATLAB e garanta que estimate_C_selfcal.m ' ...
                 'novo esta no path.']);
    end

    % DIAGNOSTICO: imprime o residuo medio por iteracao (1o ISR, todos metodos)
    % para ver se progride, fica constante, ou vira NaN.
    fprintf('\n  [diag] Residuo observavel medio por iteracao (ISR=%+d dB):\n', range_ISR_dB(1));
    fprintf('  %-10s', 'metodo');
    for it = 1:max_it, fprintf(' it%-2d ', it); end
    fprintf('\n');
    for im = 1:nMet_it
        fprintf('  %-10s', sc_labels{im});
        for it = 1:max_it
            v = resid_mean(im, it, 1);
            if isnan(v), fprintf('  NaN '); else, fprintf(' %.3f', v); end
        end
        fprintf('\n');
    end


    % --- Plot: um painel (subplot) por metodo; dentro, uma curva por ISR ---
    % Cor = ISR (degrade), estilo de linha tambem varia para diferenciar.
    fig_it = figure('Name','RMSE de DoA vs iteracoes (self-cal, varios ISR)', ...
                    'NumberTitle','off','Position',[80 80 1100 800]);
    cmap_isr   = parula(max(nISR_it,2));
    styles_isr = {'-','--',':','-.','-','--',':','-.'};
    markers_isr= {'o','s','d','^','v','>','<','p'};

    % Limites Y comuns entre paineis (para comparacao justa)
    ymin = min(rmse_iter(:), [], 'omitnan'); ymax = max(rmse_iter(:), [], 'omitnan');
    if isempty(ymin) || ~isfinite(ymin), ymin = 0.1; end
    if isempty(ymax) || ~isfinite(ymax), ymax = 100; end

    nrows = ceil(nMet_it/2);
    for im = 1:nMet_it
        subplot(nrows, 2, im);
        hold on; grid on;
        for iISR_it = 1:nISR_it
            sty = styles_isr{mod(iISR_it-1, numel(styles_isr))+1};
            mkr = markers_isr{mod(iISR_it-1, numel(markers_isr))+1};
            plot(1:max_it, squeeze(rmse_iter(im, :, iISR_it)), ...
                 'LineStyle', sty, 'Marker', mkr, 'LineWidth', 1.6, ...
                 'Color', cmap_isr(iISR_it,:), 'MarkerSize', 5, ...
                 'MarkerFaceColor', cmap_isr(iISR_it,:), ...
                 'DisplayName', sprintf('ISR=%+d dB', range_ISR_dB(iISR_it)));
        end
        set(gca,'YScale','log');
        ylim([ymin*0.8, ymax*1.2]);
        xlabel('Iteracao do self-cal'); ylabel('RMSE de \phi (graus)');
        title(sc_labels{im});
        if im == 1
            legend('Location','best','FontSize',8);
        end
    end
    sgtitle(sprintf('RMSE de DoA vs iteracoes  |  SNR=%+d dB fixo, init=identity', exp_SNR_dB));
    exportgraphics(fig_it, fullfile(outDir, 'selfcal_rmse_vs_iter_per_ISR.png'), 'Resolution', 200);
    fprintf('  -> selfcal_rmse_vs_iter_per_ISR.png\n');

    % --- GRAFICO 2: RMSE de DoA vs iteracoes (escala linear) --------------
    fig_rmse_lin = figure('Name','RMSE de DoA vs iteracoes (linear)', ...
                          'NumberTitle','off','Position',[100 100 1100 800]);
    cmap_isr_v = parula(max(nISR_it,2));
    nrows_v = ceil(nMet_it/2);
    for im = 1:nMet_it
        subplot(nrows_v, 2, im); hold on; grid on;
        for iISR_it = 1:nISR_it
            col = cmap_isr_v(iISR_it,:);
            plot(1:max_it, squeeze(rmse_iter(im,:,iISR_it)), '-o', 'Color', col, ...
                 'LineWidth', 1.6, 'MarkerSize', 4, 'MarkerFaceColor', col, ...
                 'MarkerEdgeColor','none', ...
                 'DisplayName', sprintf('ISR=%+d dB', range_ISR_dB(iISR_it)));
        end
        ylabel('RMSE de \phi (graus)'); xlabel('Iteracao'); title(sc_labels{im});
        xlim([0.5, max_it+0.5]);
        if im == 1, legend('Location','best','FontSize',8); end
    end
    sgtitle(sprintf('RMSE de DoA vs iteracoes  |  SNR=%+d dB fixo, init=identity', exp_SNR_dB));
    exportgraphics(fig_rmse_lin, fullfile(outDir, 'selfcal_rmse_vs_iter_linear.png'), 'Resolution', 200);
    fprintf('  -> selfcal_rmse_vs_iter_linear.png\n');

    % --- GRAFICO 3: RESIDUO OBSERVAVEL vs iteracoes ----------------------
    fig_resid = figure('Name','Residuo observavel vs iteracoes (self-cal)', ...
                       'NumberTitle','off','Position',[120 80 1100 800]);
    ymin_r = min(resid_mean(:),[],'omitnan'); ymax_r = max(resid_mean(:),[],'omitnan');
    if ~isfinite(ymin_r), ymin_r=0; end; if ~isfinite(ymax_r), ymax_r=1; end
    ypad = max((ymax_r-ymin_r)*0.08, 0.02);
    for im = 1:nMet_it
        subplot(nrows_v, 2, im); hold on; grid on;
        for iISR_it = 1:nISR_it
            col = cmap_isr_v(iISR_it,:);
            resid_v = squeeze(resid_mean(im,:,iISR_it));
            plot(1:max_it, resid_v, '-s', 'Color', col, 'LineWidth', 1.6, ...
                 'MarkerSize', 5, 'MarkerEdgeColor', col, 'MarkerFaceColor','none', ...
                 'DisplayName', sprintf('ISR=%+d dB', range_ISR_dB(iISR_it)));
        end
        ylim([max(ymin_r-ypad,0), min(ymax_r+ypad,1.05)]);
        ylabel('||b_{comp} - \alpha a(\hat\phi)|| / ||b_{comp}||');
        xlabel('Iteracao'); title(sc_labels{im});
        xlim([0.5, max_it+0.5]);
        if im == 1, legend('Location','best','FontSize',8); end
    end
    sgtitle(sprintf('Residuo observavel vs iteracoes  |  SNR=%+d dB fixo, init=identity', exp_SNR_dB));
    exportgraphics(fig_resid, fullfile(outDir, 'selfcal_resid_vs_iter.png'), 'Resolution', 200);
    fprintf('  -> selfcal_resid_vs_iter.png\n');

    % --- Resumo quantitativo: a iteracao que MINIMIZA o residuo observavel
    %     coincide com a que minimiza o RMSE real? -----------------------
    fprintf('\n  Coincidencia do ponto otimo (residuo observavel vs RMSE real):\n');
    fprintf('  %-10s %8s %8s %8s\n', 'metodo', 'ISR', 'it*_res', 'it*_real');
    for im = 1:nMet_it
        for iISR_it = 1:nISR_it
            [~, it_res ] = min(squeeze(resid_mean(im,:,iISR_it)));
            [~, it_real] = min(squeeze(rmse_iter(im,:,iISR_it)));
            flag = ''; if it_res == it_real, flag = '  <-- coincide'; end
            fprintf('  %-10s %+8d %8d %8d%s\n', sc_labels{im}, ...
                    range_ISR_dB(iISR_it), it_res, it_real, flag);
        end
    end
end
...
% =========================================================================
% ITEM 3a: SCATTER vs DESALINHAMENTO (sincronizacao)
% Grade: linhas = metodos self-cal, colunas = niveis de Delta (simbolos).
% Cada celula: scatter (erro_theta, erro_phi) centrado em 0, n_scatter_real
% pontos. Eixos compartilhados entre celulas. Nao-circularidade no default.
% =========================================================================
if run_scatter_sync
    fprintf('\n[Experimento] Scatter vs desalinhamento (sincronizacao)...\n');
    deltas_samp = scatter_sync_syms * sps;     % Delta em amostras
    nLvl = numel(deltas_samp);
    nMet = numel(sc_meth_list);

    % Coleta: err_th{im,il} e err_ph{im,il} vetores de n_scatter_real
    err_th = cell(nMet, nLvl);
    err_ph = cell(nMet, nLvl);
    for im = 1:nMet, for il = 1:nLvl
        err_th{im,il} = nan(n_scatter_real,1);
        err_ph{im,il} = nan(n_scatter_real,1);
    end, end

    Cmat = Coupling_matrices_real(:,:,1);      % C com nao-circ. default
    for il = 1:nLvl
        dlt = deltas_samp(il);
        fprintf('  nivel %d/%d (Delta=%g simbolos)...\n', il, nLvl, scatter_sync_syms(il));
        for rep = 1:n_scatter_real
            [phi_sig_d, phi_int_d] = deal_local_angles(min_sep_deg);
            [Xc, y_ref_c, k0c] = utils.simulate_data_uca_v3(M, exp_radius_m, lambda, ...
                phi_sig_d, phi_int_d, exp_theta, exp_theta, ...
                exp_SNR_dB, exp_ISR_dB, K, K_kw, fs, Rs, sps, alpha, span, fd, interf_type, []);
            Xc = Cmat * Xc;
            % Janela deslocada por Delta (com clipping)
            Xk = extract_window(Xc, k0c + dlt, K_kw, M, K);
            qv = y_ref_c(:);
            for im = 1:nMet
                [~, ~, ~, phi_est, ~] = estimate_C_selfcal(Xk, qv, M, exp_radius_m, lambda, ...
                    sc_meth_list{im}, selfcal_grid_deg, selfcal_max_iter, [], [], Cmat, ...
                    selfcal_damping, 'identity');
                e_ph = phi_est - phi_sig_d; e_ph = wrap180(e_ph);
                err_ph{im,il}(rep) = e_ph;
                err_th{im,il}(rep) = 0;   % theta fixo em 90 (self-cal so estima phi aqui)
            end
        end
    end

    plot_scatter_grid(err_th, err_ph, sc_labels, scatter_sync_syms, ...
        '\Delta (simbolos)', 'scatter_vs_sync.png', outDir, exp_SNR_dB, exp_ISR_dB, ...
        'Desalinhamento de sincronizacao');
    fprintf('  -> scatter_vs_sync.png\n');
end


% =========================================================================
% ITEM 3b: SCATTER vs NAO-CIRCULARIDADE de C
% Grade: linhas = metodos self-cal, colunas = niveis de C_noncirc_level.
% A perturbacao nao-circulante e' REGENERADA a cada realizacao (estatistica
% de imperfeicao). Sincronizacao perfeita (Delta=0).
% =========================================================================
if run_scatter_noncirc
    fprintf('\n[Experimento] Scatter vs nao-circularidade de C...\n');
    lvls = scatter_noncirc_lvl;
    nLvl = numel(lvls);
    nMet = numel(sc_meth_list);
    ...
    Ctx_base = compute_Ctx_for_R(fc, M, exp_radius_m, Z0);   % C circulante base
    mean_abs_C = mean(abs(Ctx_base(:)));
    for il = 1:nLvl
        lvl = lvls(il);
        fprintf('  nivel %d/%d (noncirc=%.3f)...\n', il, nLvl, lvl);
        for rep = 1:n_scatter_real
            [phi_sig_d, phi_int_d] = deal_local_angles(min_sep_deg);
            % Regenera perturbacao nao-circulante a cada realizacao
            pert = (randn(M) + 1j*randn(M))/sqrt(2) * lvl * mean_abs_C;
            Cmat = Ctx_base + pert;
            [Xc, y_ref_c, k0c] = utils.simulate_data_uca_v3(M, exp_radius_m, lambda, ...
                phi_sig_d, phi_int_d, exp_theta, exp_theta, ...
                exp_SNR_dB, exp_ISR_dB, K, K_kw, fs, Rs, sps, alpha, span, fd, interf_type, []);
            Xc = Cmat * Xc;
            Xk = Xc(:, k0c:k0c+K_kw-1);    % Delta=0
            qv = y_ref_c(:);
            for im = 1:nMet
                [~, ~, ~, phi_est, ~] = estimate_C_selfcal(Xk, qv, M, exp_radius_m, lambda, ...
                    sc_meth_list{im}, selfcal_grid_deg, selfcal_max_iter, [], [], Cmat, ...
                    selfcal_damping, 'identity');
                e_ph = wrap180(phi_est - phi_sig_d);
                err_ph{im,il}(rep) = e_ph;
                err_th{im,il}(rep) = 0;
            end
        end
    ...
    plot_scatter_grid(err_th, err_ph, sc_labels, lvls, ...
        'C_{noncirc}', 'scatter_vs_noncirc.png', outDir, exp_SNR_dB, exp_ISR_dB, ...
        'Nao-circularidade de C');
    fprintf('  -> scatter_vs_noncirc.png\n');
end
...
% =========================================================================
% Funcoes auxiliares dos experimentos dedicados
% =========================================================================
function [ps, pi_] = deal_local_angles(min_sep_deg)
%DEAL_LOCAL_ANGLES Sorteia (phi_sig, phi_int) inteiros em [-180,179] com
%   separacao circular minima de min_sep_deg graus.
    ps  = randi([-180, 179]);
    pi_ = randi([-180, 179]);
    d = abs(ps - pi_); d = min(d, 360 - d);
    while d < min_sep_deg
...
function Xk = extract_window(Xc, first, K_kw, M, K)
%EXTRACT_WINDOW Extrai janela de K_kw amostras comecando em 'first', com
%   clipping (preenche com zeros o que sair de [1,K]).
    Xk = zeros(M, K_kw);
    last = first + K_kw - 1;
    src_first = max(first, 1);
    src_last  = min(last, K);
    if src_last >= src_first
        dst_first = src_first - first + 1;
        dst_last  = src_last  - first + 1;
        Xk(:, dst_first:dst_last) = Xc(:, src_first:src_last);
    end
end

function e = wrap180(e)
%WRAP180 Envolve erro angular para [-180, 180].
    e = mod(e + 180, 360) - 180;
end

function plot_scatter_grid(err_th, err_ph, row_labels, col_levels, ...
                           col_param_name, fname, outDir, snr_dB, isr_dB, sup_title)
%PLOT_SCATTER_GRID Grade de mini-scatters: linhas = metodos, colunas =
%   niveis. Cada celula plota (erro_theta, erro_phi) centrado em 0.
%   Eixos compartilhados entre todas as celulas (mesmo xlim/ylim).
    [nMet, nLvl] = size(err_ph);

    % Determina limites comuns (percentil 98 para evitar outliers extremos)
    all_ph = []; all_th = [];
    for im = 1:nMet, for il = 1:nLvl
        all_ph = [all_ph; err_ph{im,il}(:)]; %#ok<AGROW>
        all_th = [all_th; err_th{im,il}(:)]; %#ok<AGROW>
    end, end
    ph_lim = prctile(abs(all_ph(~isnan(all_ph))), 98);
    if isempty(ph_lim) || ph_lim == 0, ph_lim = 1; end
    th_lim = prctile(abs(all_th(~isnan(all_th))), 98);
    if isempty(th_lim) || th_lim == 0, th_lim = max(1, ph_lim); end

    cmap_met = lines(nMet);
    fig = figure('Name', sup_title, 'NumberTitle','off', ...
                 'Position',[60 60 220*nLvl 200*nMet]);
    tl = tiledlayout(nMet, nLvl, 'TileSpacing','compact', 'Padding','compact');
    for im = 1:nMet
        for il = 1:nLvl
            nexttile;
            hold on; grid on;
            scatter(err_th{im,il}, err_ph{im,il}, 10, cmap_met(im,:), ...
                    'filled', 'MarkerFaceAlpha', 0.4);
            plot(0, 0, 'k+', 'MarkerSize', 10, 'LineWidth', 1.5);  % verdade (erro 0)
            xlim([-th_lim th_lim]); ylim([-ph_lim ph_lim]);
            if im == 1
                title(sprintf('%s = %g', col_param_name, col_levels(il)), ...
                      'FontWeight','normal','FontSize',9);
            end
            if il == 1
                ylabel(sprintf('%s\nerro \\phi (°)', row_labels{im}), 'FontSize',8);
            end
            if im == nMet
                xlabel('erro \theta (°)','FontSize',8);
            end
            set(gca,'FontSize',7);
        end
    end
    title(tl, sprintf('%s  |  SNR=%+d dB, ISR=%+d dB, init=identity', ...
          sup_title, snr_dB, isr_dB));
